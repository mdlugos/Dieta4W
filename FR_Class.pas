...
var
  ...
  begin
    if (Flags and flStretched) = 0 then
     begin
       Dest.Right := Dest.Left + dx;
       Dest.Bottom := Dest.Top + dy;
       Bitmap.Width := dx;
       Bitmap.Height := dy;
     end;
    with Bitmap do
...
{$WARNINGS OFF}
  ...
  begin
    if not IsPrinting then
    begin
      if (((Flags and flStretched) = 0)
{$IFDEF JPEG} and (not (aGraph is TJPEGImage)){$ENDIF})
       then
        begin
          aGraph.Width := dx;
          aGraph.Height := dy;
          Canvas.Draw(DestRect.Left, DestRect.Top, aGraph);
        end
        else
...
{$ENDIF}
    ...
    begin
      ...
      begin
        ...
        if (Flags and flPictCenter) <> 0 then
        begin
          w := (Round(ScaleX * Picture.Width) div 2) - ((DRect.Right - DRect.Left) div 2);
          h := (Round(ScaleY * Picture.Height) div 2) - ((DRect.Bottom - DRect.Top) div 2);

          tmp.Height := Picture.Bitmap.Height;
          tmp.Width := Picture.Bitmap.Width;
          tmp.Canvas.CopyRect(Rect(0 ,0,tmp.Width,tmp.Height), Picture.Bitmap.Canvas, Rect(w,h,tmp.Width+w,tmp.Height+h));
        end;
        r.Right := r.Left + Round(Picture.Width * ScaleX);
        r.Bottom := r.Top + Round(Picture.Height * ScaleY);
        if (Flags and flPictCenter) <> 0 then
          PrintBitmap(Canvas, r, tmp)
        else
          PrintGraphic(Canvas, r, Picture.Graphic);
        tmp.Free
      end;
